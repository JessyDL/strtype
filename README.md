...
The only two points you need to interact with is the `strenum::stringify<YOUR_ENUM_TYPE>()` and the specializable `strenum::enum_information`. The first is an `immediate function` that will return you an array of all uniquely named enum values, while the latter is a customization point (see example section).
